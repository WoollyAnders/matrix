...
**Tuning lives in constants.** The look is driven entirely by labeled constants — `RainField` in `src/Matrix.cs` and the `// tunables` block at the top of `wallpaper/matrix.html`: `DECAY` (tail length), `SPEED_MIN/MAX`, `CHANGE_CHANCE` (constant glyph churn), `FLASH_BOOST/DECAY` (momentary glow on change/flip), `FLIP_RATE/FLIP_CHANCE` (horizontal mirroring), `INTERRUPT_CHANCE` (wipe + re-seed a whole column), `SEG_ERASE_RATE` with weighted size tiers (`SEG_SMALL/MED/LARGE_FRAC`/`_MIN`/`_MAX`) that punch random-scale gaps in streams — a few stray chars, a mid-size segment, or a long chunk. `SPAWN_ON_ERASE` (chance an erase also seeds a fresh falling head — a "sprout" — at the gap, so a new stream regrows where code was removed: a third way the rain is built, besides wrap-around restart and post-wipe restart). Keep the values identical between the two files.
